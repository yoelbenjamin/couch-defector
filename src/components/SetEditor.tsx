...
function label(sets: SetEntry[], k: number) {
  return sets[k].warmup
    ? "warm-up"
    : `set ${sets.slice(0, k).filter((y) => !y.warmup).length + 1}`;
}
  ...
  return (
    <span
      className={cn(
        ...
        d > 0 ? "font-semibold text-foreground" : "text-muted-foreground",
        className,
      )}
    >
      {d > 0 ? `+${d}` : d < 0 ? `−${-d}` : "="}
...
export default function SetEditor({
  ...
  const setReps = (k: number, v: number) =>
    onChange(
      sets.map((y, j) => (j === k ? { ...y, reps: Math.max(0, v) } : y)),
    );
  const toggleWarmup = (k: number) =>
    onChange(sets.map((y, j) => (j === k ? { ...y, warmup: !y.warmup } : y)));
      ...
      {sets.map((s, k) => {
        ...
        return (
          <div
            key={k}
            className={cn(
              "relative w-[76px] rounded-lg border px-1.5 pt-1.5 pb-1 text-center",
              s.warmup && "bg-secondary/60",
            )}
          >
            <button
              type="button"
    ...
    <div className="">
      <div className="flex flex-wrap gap-1.5">
        {sets.map((s, k) => {
          ...
          return (
            ...
            >
              {s.warmup && (
                <span className="mr-1 text-[10px] font-normal opacity-70">
                  W
                </span>
              )}
              {s.reps}
              ...
              {p !== undefined && prevMode !== "off" && (
                <span
                  className={cn(
                    "block text-[10px] font-normal leading-3",
                    sel === k ? "opacity-70" : "text-muted-foreground",
                  )}
                >
                  {prevMode === "beside"
                    ? `last ${p}`
      ...
      {cur && (
        <div className="mt-2.5 flex items-center gap-2">
          <span className="w-14 text-[11px] font-semibold text-muted-foreground">
            {label(sets, sel)}
          </span>
          <Stepper
            value={cur.reps}
            suffix={suffix}
            onChange={(v) => setReps(sel, v)}
          />
          {prev !== undefined && prevMode !== "off" && (
      ...
      {sets.map((s, k) => (
        <div key={k} className="flex items-center gap-3 py-1.5">
          ...
          <button
            ...
            onClick={() => toggleWarmup(k)}
            className={cn(
              "w-14 text-left text-[11px] font-semibold",
              s.warmup ? "text-muted-foreground" : "",
            )}
          >
            {label(sets, k)}
          </button>
          ...
          {open === k ? (
            ...
            <button
              ...
              className={cn(
                ...
                done[k] && "text-muted-foreground line-through",
              )}
            >
              {s.reps}
              {suffix}
