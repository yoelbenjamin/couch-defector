...
export default function Settings() {
  ...
  const customSlots = program.cycle.flatMap((c) =>
    'rest' in c && c.rest ? [] : c.day.slots.filter((s): s is Slot & { kind: 'custom' } => s.kind === 'custom'),
  )
      ...
      <Tray>
        ...
        <Card variant="inset" className="gap-3">
          <CardHeader>
            ...
            <CardDescription className="text-xs">
              Where you are on each of the six movements. Pick the hardest step you can do for at least a few clean reps.
            </CardDescription>
          </CardHeader>
        ...
        <Card variant="inset" className="gap-3">
          <CardHeader>
            ...
            <CardDescription className="text-xs">
              Your step on each Trifecta hold. Rest days show these three. Pick the version you can hold comfortably, never the hardest.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-2">
            {HOLD_IDS.map((h) => (
