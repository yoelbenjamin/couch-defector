  ...
  schedule.forEach((slotDay, i) => {
    ...
    const entries: Entry[] = day.slots.map((s): Entry => {
      if (s.kind === 'progression') {
        const st = (state[s.progression] ??= {
          step: o.startSteps?.[s.progression] ?? 1,
          reps: getStep(s.progression, o.startSteps?.[s.progression] ?? 1).start,
        })
        const step = getStep(s.progression, st.step)
        let reps = st.reps
        if (isLast && o.peakLast) reps = step.goal.reps
        const hard = Array.from({ length: Math.max(program.workSets, step.goal.sets) }, (_, k) => ({
          reps: Math.max(1, reps - (k > 0 && rand() < 0.5 ? 1 : 0)),
        }))
        const entry: Entry = {
          slotKey: s.key,
          name: step.name,
...
export const SCENARIOS: Scenario[] = [
  ...
  { id: 'new-account', group: 'Account', name: 'New account', description: 'Signed in, no program picked. Onboarding.', build: () => empty(null) },
  {
    ...
    group: 'Lifecycle',
    name: 'Day one',
    description: 'Program picked, nothing logged yet.',
    build: () => empty('new-blood', { squat: 2, pullup: 1, pushup: 3, legraise: 2 }),
  },
  {
    id: 'due-today',
    group: 'Lifecycle',
    name: 'Workout due',
    description: 'Veterano, three weeks in. A session is due unless it is Sunday.',
    build: () => history({ programId: 'veterano', weeks: 3, daysSinceLast: 2 }),
  },
  {
    id: 'rest-day',
    group: 'Lifecycle',
    name: 'Rest day',
    description: 'New Blood: only Monday and Friday train, so most days rest.',
    build: () => history({ programId: 'new-blood', weeks: 3, daysSinceLast: 1 }),
  },
  {
    id: 'done-today',
    group: 'Lifecycle',
    name: 'Done today',
    description: 'Already logged this morning.',
    build: () => history({ programId: 'veterano', weeks: 3, daysSinceLast: 0 }),
  },
  {
    id: 'ready-up',
    group: 'Lifecycle',
    name: 'Ready to move up',
    description: 'Last session hit the goal on every exercise.',
    build: () => history({ programId: 'good-behavior', weeks: 5, daysSinceLast: 0, peakLast: true }),
  },
  {
    id: 'back-to-back',
    group: 'Lifecycle',
    name: 'Back-to-back day',
    description: 'Solitary Confinement, six days a week. Trained yesterday, due again today.',
  ...
  },
  {
    ...
    group: 'Lifecycle',
    name: 'Long break',
    description: 'Sixteen days off. What does coming back feel like?',
    build: () => history({ programId: 'good-behavior', weeks: 6, daysSinceLast: 16 }),
  },
  {
    id: 'veteran',
    group: 'Volume',
    ...
    build: () =>
      history({ programId: 'veterano', weeks: 20, daysSinceLast: 2, startSteps: { pushup: 4, squat: 4, pullup: 3, legraise: 3, bridge: 2, handstand: 1 } }),
  },
  {
    id: 'advanced',
    group: 'Volume',
    ...
    build: () =>
      history({ programId: 'supermax', weeks: 8, daysSinceLast: 1, startSteps: { pushup: 7, squat: 7, pullup: 6, legraise: 6, bridge: 5, handstand: 4 } }),
  },
]
